...
public class EditPersonPage extends PageServlet 
{ 
  ...
  editPerson_Template templateservlet = new editPerson_Template();
  ...
  private String arg13_id;

  public void setArg13_id(String v)
  { 
    this.arg13_id = v;
  }

  public String getArg13_id()
  { 
    return this.arg13_id;
  }
  ...
  { 
    exampleapp.domain.Gender maleGender = null;
    maleGender = GlobalVariables.getMaleGender(hibSession);
    env.putVariable("maleGender", maleGender);
    exampleapp.domain.Gender femaleGender = null;
    femaleGender = GlobalVariables.getFemaleGender(hibSession);
  ...
  { 
    combinedparams.append("arg" + "=" + utils.HTMLFilter.filter(arg13.getNaturalId().toString()) + "&");
  }

  public void hiddenParams(java.lang.StringBuffer combinedparams)
  { 
    combinedparams.append("<input type=\"hidden\" name=\"" + "arg" + "\" value=\"" + utils.HTMLFilter.filter(arg13.getNaturalId().toString()) + "\" />");
  }
  ...
  { 
    if(arg13_id != null)
    { 
      arg13_id = utils.URLFilter.unfilter(arg13_id.toString());
      List<exampleapp.domain.Person> results = hibSession.createCriteria(exampleapp.domain.Person.class).add(org.hibernate.criterion.Restrictions.naturalId().set("_username", arg13_id)).list();
      if(results.size() == 0)
      { 
        System.out.println("No entity with this identity found: " + arg13_id);
        throw new RuntimeException("No entity with this identity found: " + arg13_id);
      }
      else
        if(results.size() > 1)
        { 
          System.out.println("Multiple entities with this identity found: " + arg13_id);
          throw new RuntimeException("Multiple entities with this identity found: " + arg13_id);
        }
        else
        { 
          arg13 = results.get(0);
